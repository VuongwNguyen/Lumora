...
- **Tên tệp trong `public/` không được trùng luật chặn chung của EasyList/EasyPrivacy.** `scene/beacon.js` từng bị uBlock Origin nuốt bằng luật `/beacon.js` — không neo tên miền, khớp chuỗi con, nên bắn trúng cả localhost lẫn production trên máy mọi khách có uBlock. Server vẫn 200, log vẫn sạch; với ES module thì một import bị chặn giết cả module gốc và người dùng chỉ thấy màn đen. `npm run test:adblock` đối chiếu tên tệp với 1009 luật trích sẵn trong `tests/fixtures/`. Định danh bên trong code thì vô hại — bộ lọc chỉ nhìn URL.
- **`/view/` đọc HTML của 5 universe MỘT LẦN lúc khởi động** (`TEMPLATE_HTML` trong `index.js:106`), và nodemon không theo dõi `.html`. Sửa file HTML rồi chụp qua `/view/` sẽ chụp bản cũ. `touch index.js` để nodemon nạp lại. Truy thẳng `/{template}/` thì thấy bản mới ngay — thêm một lý do nữa để hai đường không cho cùng kết quả.
